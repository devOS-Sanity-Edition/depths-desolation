...
public class TreeodeConfiguration extends DecoratedGeodeConfiguration {
	public static final Codec<TreeodeConfiguration> CODEC = RecordCodecBuilder.create(
			instance -> instance.group(
					...
					Codec.list(BlockState.CODEC).fieldOf("floor_decor").forGetter(config -> config.floorDecor)
			).apply(instance, TreeodeConfiguration::new)
	);

	public final Holder<ConfiguredFeature<?, ?>> treeFeature;
	public final List<BlockState> floorDecor;

	public TreeodeConfiguration(GeodeConfiguration geodeConfig, Holder<ConfiguredFeature<?, ?>> treeFeature, List<BlockState> floorDecor) {
		super(geodeConfig);
